...
export default class AlertView extends React.Component {
                ...
                }
                {
                    alertType === 'never'
                        ? (
                            ...
                            >
                                <Image source={neverState ? modalNeverSelected : modalNeverUnselected} />
                                <Text style={[_styles.content, { marginLeft: 8 }]}>
                                    {neverText}
                                </Text>
                            </TouchableOpacity>
                        ) : null
    ...
    renderActions = (_styles) => {
        ...
        const flexDirection = buttonDirection === 'auto' ? (buttons.length > 2 ? 'column' : 'row') : buttonDirection
        const containerStyles = { flexDirection }
        const buttonContainerStyles = flexDirection === 'row'
            ? [_styles.buttonContainer, { flex: 1 }] : _styles.buttonContainer
        const buttonViewStyles = flexDirection === 'row'
            ? { flex:1, flexDirection:'row' } : { flexDirection: 'column' }
            ...
            <View style={containerStyles}>
                {
                    buttons.map((item, index) => (
                        ...
                        <View key={`Button${index}`} style={buttonViewStyles}>
                            ...
                            >
                                <Text
                                    style={[_styles.action,item.color ? { color:item.color } : undefined]}
                        ...
                        return (
                            ...
                            >
                                <View style={_styles.dialogContainer}>
                                    ...
                                    {actions.length > 0 ? <View style={_styles.horizontalLine} /> : null}
